...
function makeSelectOptions(sel, items, placeholder, selectedValue){
  if(!sel) return;
  const wanted = selectedValue == null ? String(sel.value || "") : String(selectedValue || "");
  sel.innerHTML = "";
  const opt0=document.createElement("option");
  opt0.value=""; opt0.textContent = placeholder || "Vsi";
  sel.appendChild(opt0);
  let hasWanted = wanted === "";
  (items||[]).forEach(x=>{
    const o=document.createElement("option");
    o.value=x.id;
    o.textContent=x.name;
    if(o.value === wanted) hasWanted = true;
    sel.appendChild(o);
  });
  sel.value = hasWanted ? wanted : "";
}
...
async function homePage(){
  ...
  const coachSearch = document.querySelector("[data-filter-coach-search]");
  const locSearch = document.querySelector("[data-filter-location-search]");
  const playerSearch = document.querySelector("[data-filter-player-search]");
  const coachCombo = document.querySelector("[data-filter-coach-combo]");
  const locCombo = document.querySelector("[data-filter-location-combo]");
  const playerCombo = document.querySelector("[data-filter-player-combo]");
  const coachDropdown = document.querySelector("[data-filter-coach-dropdown]");
  const locDropdown = document.querySelector("[data-filter-location-dropdown]");
  const playerDropdown = document.querySelector("[data-filter-player-dropdown]");

  const allCoaches = db.coaches || [];
  const allLocations = db.locations || [];
  const allPlayers = db.players || [];
  function wireFilterCombo(cfg){
    const sel = cfg.sel;
    const search = cfg.search;
    const combo = cfg.combo;
    const dropdown = cfg.dropdown;
    const items = cfg.items || [];
    const allLabel = cfg.allLabel || "Vsi";

    makeSelectOptions(sel, items, allLabel);
    if(!sel){
      return { combo: null, close: ()=>{} };
    }
    ...
    function renderDropdown(forceAll){
      if(!dropdown) return;
      const q = forceAll ? "" : String(search?.value || "").trim().toLowerCase();
      const selectedId = sel.value || "";
      ...
      if(filtered.length===0){
        ...
        filtered.forEach(x=>{
          ...
          if(selectedId && selectedId===x.id) btn.classList.add("selected");
    ...
    if(search){
      const open = ()=>renderDropdown(true);
      search.addEventListener("focus", open);
      search.addEventListener("click", open);
      search.addEventListener("input", ()=>renderDropdown(false));
      search.addEventListener("keydown", (e)=>{
        ...
        if(e.key==="Enter"){
          const first = dropdown?.querySelector("button.combo-item");
          if(first){
            e.preventDefault();
    ...
    if(dropdown){
      dropdown.addEventListener("click", (e)=>{
        const btn = e.target.closest("button.combo-item");
        if(!btn) return;
        selectValue(btn.dataset.value || "");
      });
    }

    return { combo, close: closeDropdown };
  }

  const coachFilter = wireFilterCombo({
    sel: coachSel,
    search: coachSearch,
    combo: coachCombo,
    dropdown: coachDropdown,
    items: allCoaches,
    allLabel: "Vsi trenerji"
  });
  const locationFilter = wireFilterCombo({
    sel: locSel,
    search: locSearch,
    combo: locCombo,
    dropdown: locDropdown,
    items: allLocations,
    allLabel: "Vse lokacije"
  });
  const playerFilter = wireFilterCombo({
    sel: playerSel,
    search: playerSearch,
    combo: playerCombo,
    dropdown: playerDropdown,
    items: allPlayers,
    allLabel: "Vsi igralci"
  });
  const comboFilters = [coachFilter, locationFilter, playerFilter];
  ...
  function render(){
    ...
    if(monthLabel){
      const monthName = viewMonth.toLocaleString("sl-SI", { month: "long" }).toUpperCase();
  ...
  document.addEventListener("click", (e)=>{
    ...
    comboFilters.forEach(f=>{
      if(f?.combo && !f.combo.contains(t)) f.close();
    });
...
async function adminPage(){
  ...
  if(importBtn) importBtn.addEventListener("click", async ()=>{
    ...
    try{
      ...
      alert("Napaka pri uvozu: " + (e && e.message ? e.message : e));
    }
  });
